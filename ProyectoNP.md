---
title: "Efecto de Marcar Primero y Período del Partido en la Copa del Mundo y la Eurocopa de la UEFA"
author: "Valentina Muñoz Palma, Kevin Leonardo Chaparro Reyes, Paula Margarita Triana Ancinez"
date: "2025-05-19"
output:
...
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE, fig.path = "ProyectoNP_files/figure-gfm/", fig.cap = TRUE)
# Nota: La opción fig.path y fig.cap son más relevantes para cuando se renderiza a HTML o PDF directamente.
# GitHub usualmente maneja las imágenes que están enlazadas.
# La opción echo = TRUE es por defecto, pero se puede poner explícitamente.
# warning = FALSE y message = FALSE ocultan mensajes y advertencias de R en el output.
Fragmento de código

# install.packages("tidyverse") # Comentado para no ejecutar en cada knit si ya está instalado
# install.packages("readxl")    # Comentado
# install.packages("gt")        # Comentado
# install.packages("car")       # Comentado
# install.packages("tidyr")     # Ya es parte de tidyverse
# install.packages("purrr")     # Ya es parte de tidyverse
Fragmento de código
...
library(gt)
library(car)
# library(tidyr) # No es necesario cargarla explícitamente si tidyverse está cargado
# library(purrr) # No es necesario cargarla explícitamente si tidyverse está cargado
Este texto introductorio explica el propósito del documento y los pasos generales que se seguirán. Markdown es una sintaxis de formato simple para crear documentos HTML, PDF y MS Word. Para más detalles sobre el uso de R Markdown, consulta http://rmarkdown.rstudio.com.

Cuando haces clic en el botón Knit, se generará un documento que incluye tanto el contenido como la salida de cualquier fragmento de código R incrustado dentro del documento. Puedes incrustar un fragmento de código R como este:

Lectura y Preparación de Datos
A continuación, se cargan los datos desde un archivo Excel.
...
Aquí se realiza la conversión de tipos de datos para las columnas numéricas y categóricas.
...
str(datos_goles)
Se filtran los partidos que terminaron 0-0 en tiempo regular y se crea una nueva variable.
...
Ten en cuenta que el parámetro echo = FALSE se añadió al trozo de código para evitar que se imprima el código R que generó el gráfico (esto es un texto genérico de R Markdown, puedes modificarlo o eliminarlo).

Cálculo del porcentaje de victorias al marcar primero.
...
Porcentaje de veces que ganó si anotaba primero, dependiendo del equipo.
...
Gráfico del porcentaje de victorias al marcar primero.
...
Distribución de la cantidad de primeros goles por período.
...
ggplot(datos_goles, aes(x = factor(P_GOL_1, levels = 1:6, labels = periodo_labels))) +
  geom_histogram(stat = "count", fill = "#342BA8", color = "white") + # Nota: geom_histogram con stat="count" es similar a geom_bar()
  labs(
...
goles_por_periodo_agregado <- datos_goles %>%
  ...
  mutate(
    periodo_factor = factor(
      periodo_txt,
      levels = c("goles_p1", "goles_p2", "goles_p3", "goles_p4", "goles_p5", "goles_p6", "goles_pr1", "goles_pr2"), # Asegúrate que estos niveles sean correctos para tus datos
      labels = c("0-15", "16-30", "31-45", "46-60", "61-75", "76-90", "Prór. 1", "Prór. 2") # Ajusta si es necesario
    )
  )
...
combined_data <- bind_rows(data_by_year, data_torneo_totals, data_grand_total) %>%
  mutate(
    ...
    Order_Display_Column = ifelse(Display_Column == "Total", 2, 1) # "Total" de torneo después de años
  ) %>%
  arrange(Order_TORNEO_Group, Order_Display_Column, Display_Column) %>%
  mutate(
    TORNEO_Group = case_when( # Renombrar para gt
...
final_table <- combined_data %>%
  ...
  cols_label(Display_Column = "Year") %>% # Etiqueta para la columna de stubs (años)
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = Display_Column, # Ahora es la columna de stubs
      rows = Display_Column == "Total" # Condición para aplicar el estilo
    )
  ) %>%
  tab_style( # Estilo para la fila "Total General" (que tiene "" en Display_Column)
    style = cell_text(weight = "bold"),
    locations = cells_row_groups(groups = "Total") # Para la fila del grupo "Total"
  )
...
tabla_datos_ancha <- all_percentages_data %>%
  ...
  mutate(
    Competición_Order = case_when( # Para ordenar Euro copa, Mundial, Total
...
Creación de nuevas variables de goles por período.
...
datos_goles=datos_goles%>%mutate(
  ...
  goles_p5 = P5_L + P5_V,
  goles_p6 = P6_L + P6_V)
Análisis Inferencial y Tablas Complejas
Generación de la Tabla 3.
...
if(nrow(mundial_2018_stats_fisher) == 1 && nrow(mundial_2022_stats_fisher) == 1) {
  ...
  tabla_contingencia_mundial <- matrix(
    ...
    nrow = 2, byrow = TRUE, # Cambiado a TRUE para el orden usual (Filas: Año1, Año2; Columnas: Éxitos, Fracasos)
    dimnames = list(
...
ejecutar_shapiro_wilk <- function(valores_columna) {
  valores_limpios <- valores_columna[!is.na(valores_columna)]
   if (length(valores_limpios) >= 3 && length(valores_limpios) <= 5000) { # Shapiro-Wilk recomendado para n entre 3 y 5000
  ...
  } else if (length(valores_limpios) > 5000) { # Si N > 5000, Shapiro no es ideal
    return(
      tibble(
        N_Observaciones = length(valores_limpios),
        W_Statistic = NA_real_,
        P_Value = NA_real_ # Podrías considerar otras pruebas como Kolmogorov-Smirnov aquí
      )
    )
  } else { # Si N < 3, no se puede ejecutar Shapiro
    return(
      tibble(
        N_Observaciones = length(valores_limpios),
        W_Statistic = NA_real_,
...
Gráficos Q-Q para confirmar normalidad.
...
combinaciones_qq <- tribble(
  ...
  "MUNDIAL",    2022,      "TOTAL_GN" #Ejemplo adicional
)
...
if (exists("resultados_normalidad") && nrow(resultados_normalidad) > 0) {
  combinaciones_qq_con_stats <- combinaciones_qq %>%
    mutate(
      TORNEO = factor(TORNEO_str, levels = levels(resultados_normalidad$TORNEO)), # Asegurar que los niveles del factor coincidan
      YEAR = as.double(YEAR_num), # Asegurar tipo numérico para YEAR
      Variable_Testeada = Variable_str
    ) %>%
    left_join(resultados_normalidad, by = c("TORNEO", "YEAR", "Variable_Testeada")) %>%
    filter(!is.na(P_Value)) # Solo graficar si hay P_Value (es decir, se pudo hacer Shapiro)
  ...
  if (nrow(combinaciones_qq_con_stats) == 0) {
    ...
    for (i in 1:nrow(combinaciones_qq_con_stats)) {
      ...
      if (length(datos_para_grafico_qq) < 3) { # Necesario para geom_qq y geom_qq_line
...
comparaciones_por_torneo <- list(
  ...
  `EURO COPA` = list(años = c(2020, 2024), etiqueta_comp = "Eurocopa: 2020 vs 2024") # Usar acentos graves si el nombre tiene espacios
)
...
for (nombre_torneo_loop in names(comparaciones_por_torneo)) { 
  ...
  for (nombre_variable_loop in variables_numericas_a_testear) { 
    datos_filtrados_levene <- data %>%
      filter(
        TORNEO == nombre_torneo_loop, # Usar el nombre original del factor TORNEO
        YEAR %in% años_a_comparar_loop
      ) %>%
      select(YEAR, Valor = all_of(nombre_variable_loop)) %>%
      mutate(YEAR = as.factor(YEAR)) %>% # LeveneTest necesita un factor para el grupo
      drop_na(Valor) 

    # Calcular N y varianza por grupo para Levene
    stats_por_año_levene <- datos_filtrados_levene %>%
      group_by(YEAR) %>%
      summarise(
        n_obs = n(),
        varianza = var(Valor, na.rm = TRUE) # var() devuelve NA si n_obs < 2
      ) %>%
      ungroup()
    ...
    if (nrow(stats_año1_levene) == 0 || nrow(stats_año2_levene) == 0 || 
        stats_año1_levene$n_obs < 2 || stats_año2_levene$n_obs < 2) { # Levene necesita al menos 2 obs por grupo
      ...
      if (is.na(var1_levene) || is.na(var2_levene)) { # Esto puede ocurrir si n_obs < 2
        nota_val_levene <- "Varianza no calculable (n<2 o todos NAs) en uno o ambos grupos."
      } else if (var1_levene == 0 && var2_levene == 0) {
        p_val_val_levene <- 1.0 # Si ambas varianzas son cero, son homocedásticas.
        nota_val_levene <- "Ambos grupos tienen varianza cero (homocedásticos)."
      } else if (var1_levene == 0 || var2_levene == 0) {
        p_val_val_levene <- 0.00000001 # Si una es cero y la otra no, son heterocedásticas.
        nota_val_levene <- "Un grupo tiene varianza cero y el otro no (heterocedásticos)."
      } else {
        # Ejecutar LeveneTest
        resultado_test_obj_levene <- tryCatch({
          leveneTest(Valor ~ YEAR, data = datos_filtrados_levene, center = median) # Usar median es más robusto
        }, error = function(e) {
    ...
    temp_resultado_levene <- tibble(
      ...
      N_Total_Año1 = n_original_año1_levene, # N original antes de filtrar NAs para la variable actual
      N_Efectivo_Año1 = ifelse(nrow(stats_año1_levene) == 0, 0, stats_año1_levene$n_obs), # N usado en el test de Levene
...
resultados_levene_con_conclusion <- resultados_levene_pares_años %>%
  mutate(
    Conclusion_Homocedasticidad = case_when(
      grepl("varianza cero", Nota_Levene, ignore.case = TRUE) ~ Nota_Levene, # Casos especiales manejados
      ...
      is.na(P_Value_Levene) ~ Nota_Levene, # Si hay nota y P_Value es NA
      P_Value_Levene >= 0.05 ~ "Homocedasticidad (p >= 0.05)",
      P_Value_Levene < 0.05 ~ "Heterocedasticidad (p < 0.05)",
      TRUE ~ "Revisar caso" # Para cualquier otro caso no cubierto
    )
  )
...
for (nombre_torneo_mw in names(comparaciones_por_torneo)) {
  ...
  for (nombre_variable_mw in variables_numericas_a_testear) {
    datos_filtrados_mw <- data %>%
      ...
      mutate(YEAR_factor = as.factor(YEAR)) %>% # wilcox.test necesita un factor para el grupo
      drop_na(Valor) 

    # Calcular N y mediana por grupo para Mann-Whitney
    ...
    if (n_efectivo_año1 < 1 || n_efectivo_año2 < 1) { # Necesita al menos una observación por grupo
      ...
      resultado_test_obj_mw <- tryCatch({
        wilcox.test(Valor ~ YEAR_factor, data = datos_filtrados_mw, 
                    exact = FALSE, # No calcular p-valor exacto si hay empates o N grande
                    conf.int = FALSE) # No calcular intervalo de confianza para la diferencia de localización
      }, error = function(e) {
        return(list(error_message = as.character(e$message)))
      })
...
resultados_mann_whitney_con_conclusion <- resultados_mann_whitney %>%
  mutate(
    Conclusion_MW = case_when(
      !is.na(Nota_MW) & Nota_MW != "" ~ paste("No se pudo determinar:", Nota_MW), # Si hay nota de error
...
Datos descriptivos para Tabla 3.

Fragmento de código

#datos descriptivos para armar la tabla################################################################
# Paso 1: Asegurar que 'data' está en formato largo para las variables a analizar
data_long_para_medianas <- data %>%
  pivot_longer(
    cols = all_of(variables_numericas_a_testear), # Usa las mismas variables que para normalidad/homocedasticidad
    names_to = "Variable_Analizada",
    values_to = "Valor"
  ) %>%
  drop_na(Valor) # Importante: las medianas se calculan sobre valores no-NA

# Paso 2: Calcular medianas y N para los años individuales especificados en 'comparaciones_por_torneo'
# (Asegúrate que 'comparaciones_por_torneo' esté definido como antes)
# Ejemplo de 'comparaciones_por_torneo' si no está definido:
# comparaciones_por_torneo <- list(
#   MUNDIAL = list(años = c(2018, 2022), etiqueta_comp = "Mundial: 2018 vs 2022"),
...
medianas_años_individuales <- data_long_para_medianas %>%
  ...
  summarise(
    Mediana = median(Valor, na.rm = TRUE), # na.rm es redundante por el drop_na anterior, pero no daña
    N_Obs = n(), # n() cuenta las filas después del drop_na
    .groups = 'drop'
  ) %>%
  mutate(Grupo_Descriptor = as.character(YEAR)) # Para usar como 'stub' en la tabla gt
...
for (torneo_actual in names(comparaciones_por_torneo)) {
  ...
  medianas_temp_total <- data_long_para_medianas %>%
    ...
    group_by(TORNEO, Variable_Analizada) %>% # Agrupar por TORNEO y Variable para el total del torneo
    summarise(
    ...
    mutate(
      YEAR = NA_real_, # No hay un 'YEAR' específico para el total
...
# Asegurar que las columnas son consistentes para el bind_rows
...
# Unir medianas de años individuales con medianas de totales por torneo
tabla_descriptivos_medianas <- bind_rows(
  medianas_años_individuales,
  medianas_totales_por_torneo_df
) %>%
  arrange(Variable_Analizada, TORNEO, Grupo_Descriptor) # Ordenar para una visualización más clara
...
definicion_columnas_tabla3 <- definicion_columnas_tabla3 %>%
  mutate(Nombre_Col_df = make.names(Encabezado_GT)) # Nombres seguros para R
...
formatear_MW <- function(W_stat, p_val) {
  ...
  p_val_formateado <- format.pval(p_val, digits = 2, eps = 0.001) # Muestra "<0.001" si es muy pequeño
  estrellas_sig <- case_when(p_val < 0.001 ~ "***", p_val < 0.01 ~ "**", p_val < 0.05 ~ "*", TRUE ~ "")
  paste0("W=",round(W_stat, 0), estrellas_sig, " (p=", p_val_formateado, ")") #W= añadido
}
...
formatear_Fisher <- function(OR, OR_CI_L, OR_CI_U, p_val) {
  ...
  estrellas_sig <- case_when(p_val < 0.001 ~ "***", p_val < 0.01 ~ "**", p_val < 0.05 ~ "*", TRUE ~ "")
  or_texto <- paste0("OR=", sprintf("%.2f", OR), " [", sprintf("%.2f", OR_CI_L), "-", sprintf("%.2f", OR_CI_U), "]") #OR= añadido
...
# Años para cada torneo
años_mundial <- comparaciones_por_torneo$MUNDIAL$años
años_euro <- comparaciones_por_torneo$`EURO COPA`$años


# --- SECCIÓN MUNDIAL ---
torneo_actual_label <- "Mundial" # Cómo se verá en la tabla gt
torneo_actual_data <- "MUNDIAL"  # Cómo está en tus datos (factor TORNEO)
años_actuales <- años_mundial
etiqueta_comparacion_actual <- comparaciones_por_torneo$MUNDIAL$etiqueta_comp # "Mundial: 2018 vs 2022"

# Fila para Mundial - Año 1 (e.g., 2018)
...
for(j in 1:nrow(definicion_columnas_tabla3)){
  ...
  if(tipo_desc == "porcentaje"){
    ...
    # Asegurar que datos_fisher_m no está vacío y tiene las columnas necesarias
    valor_display <- if(nrow(datos_fisher_m) == 1) formatear_porcentaje_N(datos_fisher_m$Prop_Año1_Porc, datos_fisher_m$Total_Año1) else "-"
  } else { # tipo_desc == "mediana"
    datos_desc_m <- tabla_descriptivos_medianas %>% 
      filter(TORNEO == torneo_actual_data, YEAR == años_actuales[1], Variable_Analizada == var_int)
    valor_display <- if(nrow(datos_desc_m) == 1) formatear_mediana_N(datos_desc_m$Mediana, datos_desc_m$N_Obs) else "-"
  }
...
# Fila para Mundial - Año 2 (e.g., 2022)
...
for(j in 1:nrow(definicion_columnas_tabla3)){
  ...
  if(tipo_desc == "porcentaje"){
    datos_fisher_m <- tabla_final_fisher %>% filter(Torneo_Base == torneo_actual_data, Comparacion == etiqueta_comparacion_actual)
    valor_display <- if(nrow(datos_fisher_m) == 1) formatear_porcentaje_N(datos_fisher_m$Prop_Año2_Porc, datos_fisher_m$Total_Año2) else "-"
  } else {
    datos_desc_m <- tabla_descriptivos_medianas %>% 
...
for(j in 1:nrow(definicion_columnas_tabla3)){
  ...
  if(tipo_desc == "porcentaje"){
    # Calcular total para PORC_MARCAR_PRIMERO_GANAR para el torneo actual
    ...
    valor_display <- if(nrow(total_efecto_stats) == 1) formatear_porcentaje_N(total_efecto_stats$PORC_Total, total_efecto_stats$N_Total) else "-"
  } else {
...
# Fila para Mundial - Test Estadístico (Mann-Whitney U / Fisher)
...
for(j in 1:nrow(definicion_columnas_tabla3)){
  ...
  if(tipo_desc == "porcentaje"){
    datos_fisher_m <- tabla_final_fisher %>% filter(Torneo_Base == torneo_actual_data, Comparacion == etiqueta_comparacion_actual)
    valor_display <- if(nrow(datos_fisher_m) == 1) formatear_Fisher(datos_fisher_m$Odds_Ratio, datos_fisher_m$OR_CI_Lower, datos_fisher_m$OR_CI_Upper, datos_fisher_m$P_Valor) else "-"
  } else {
    ...
    valor_display <- if(nrow(datos_mw_m) == 1) formatear_MW(datos_mw_m$W_Statistic_MW, datos_mw_m$P_Value_MW) else "-"
  }
...
# --- SECCIÓN EUROCOPA ---
torneo_actual_label <- "Eurocopa" # Cómo se verá en la tabla gt
torneo_actual_data <- "EURO COPA"   # Cómo está en tus datos (factor TORNEO)
años_actuales <- años_euro
etiqueta_comparacion_actual <- comparaciones_por_torneo$`EURO COPA`$etiqueta_comp # "Eurocopa: 2020 vs 2024"

# Fila para Eurocopa - Año 1 (e.g., 2020)
...
for(j in 1:nrow(definicion_columnas_tabla3)){
  ...
  if(tipo_desc == "porcentaje"){
    ...
    valor_display <- if(nrow(datos_fisher_e) == 1) formatear_porcentaje_N(datos_fisher_e$Prop_Año1_Porc, datos_fisher_e$Total_Año1) else "-"
  } else {
    datos_desc_e <- tabla_descriptivos_medianas %>% 
      filter(TORNEO == torneo_actual_data, YEAR == años_actuales[1], Variable_Analizada == var_int)
    valor_display <- if(nrow(datos_desc_e) == 1) formatear_mediana_N(datos_desc_e$Mediana, datos_desc_e$N_Obs) else "-"
  }
  fila_euro_año1[[col_df_name]] <- valor_display
}
filas_para_tabla_gt <- append(filas_para_tabla_gt, list(fila_euro_año1))

# Fila para Eurocopa - Año 2 (e.g., 2024)
...
for(j in 1:nrow(definicion_columnas_tabla3)){
  ...
  if(tipo_desc == "porcentaje"){
    datos_fisher_e <- tabla_final_fisher %>% filter(Torneo_Base == torneo_actual_data, Comparacion == etiqueta_comparacion_actual)
    valor_display <- if(nrow(datos_fisher_e) == 1) formatear_porcentaje_N(datos_fisher_e$Prop_Año2_Porc, datos_fisher_e$Total_Año2) else "-"
  } else {
    datos_desc_e <- tabla_descriptivos_medianas %>% 
      filter(TORNEO == torneo_actual_data, YEAR == años_actuales[2], Variable_Analizada == var_int)
    valor_display <- if(nrow(datos_desc_e) == 1) formatear_mediana_N(datos_desc_e$Mediana, datos_desc_e$N_Obs) else "-"
...
for(j in 1:nrow(definicion_columnas_tabla3)){
  ...
  if(tipo_desc == "porcentaje"){
    total_efecto_stats_e <- efecto_anual_counts %>%
      ...
      mutate(PORC_Total = ifelse(sum_partidos > 0, (sum_victorias / sum_partidos) * 100, 0),
             N_Total = sum_partidos)
    valor_display <- if(nrow(total_efecto_stats_e) == 1) formatear_porcentaje_N(total_efecto_stats_e$PORC_Total, total_efecto_stats_e$N_Total) else "-"
  } else {
...
# Fila para Eurocopa - Test Estadístico
...
for(j in 1:nrow(definicion_columnas_tabla3)){
  ...
  if(tipo_desc == "porcentaje"){
    datos_fisher_e <- tabla_final_fisher %>% filter(Torneo_Base == torneo_actual_data, Comparacion == etiqueta_comparacion_actual)
    valor_display <- if(nrow(datos_fisher_e) == 1) formatear_Fisher(datos_fisher_e$Odds_Ratio, datos_fisher_e$OR_CI_Lower, datos_fisher_e$OR_CI_Upper, datos_fisher_e$P_Valor) else "-"
  } else {
    datos_mw_e <- resultados_mann_whitney_con_conclusion %>% 
      filter(Comparacion == etiqueta_comparacion_actual, Variable_Testeada == var_int)
    valor_display <- if(nrow(datos_mw_e) == 1) formatear_MW(datos_mw_e$W_Statistic_MW, datos_mw_e$P_Value_MW) else "-"
  }
...
#View(df_para_tabla3_final) # Para inspeccionar antes de pasar a gt

#######
# Crear los nombres de las columnas para gt a partir de la definición
# (los encabezados definidos en definicion_columnas_tabla3)
nombres_columnas_gt <- setNames(
  definicion_columnas_tabla3$Encabezado_GT,   # Los valores son los encabezados bonitos
  definicion_columnas_tabla3$Nombre_Col_df    # Los nombres son los nombres de columna seguros del df
)

tabla3_gt <- df_para_tabla3_final %>%
  gt(groupname_col = "GT_Torneo_Grupo", rowname_col = "GT_Etiqueta_Fila") %>%
  cols_label(
    .list = nombres_columnas_gt # Pasa la lista nombrada para las etiquetas
  ) %>%
  tab_header(
  ...
  cols_align(
    align = "center",
    columns = all_of(definicion_columnas_tabla3$Nombre_Col_df) # Usar los nombres seguros del df
  ) %>%
  tab_style( # Poner en cursiva la fila de Test Estadístico
    style = list(
  ...
  tab_style( # Poner en negrita la fila de Total
    style = list(
  ...
  tab_footnote(
    footnote = md("* p<0.05; ** p<0.01; *** p<0.001") # Nota al pie general
  )

# Imprimir la tabla final
print(tabla3_gt)
Probabilidades de ganar y gráficos relacionados.
...
probabilidades_ganar <- datos_goles %>%
  filter(P_GOL_1 %in% c("1", "2", "3", "4", "5", "6")) %>% # Solo periodos de tiempo reglamentario
  # Agrupamos por quién anotó el primer gol y en qué período
  group_by(PRIM_GOL, P_GOL_1) %>%
  summarise(
    total_partidos_condicion = n(),
    veces_gano_quien_anoto_primero = sum(PRIM_GOL == GANO, na.rm = TRUE), # na.rm por si GANO es NA
    .groups = 'drop'
  ) %>%
  mutate(
    prob_ganar = ifelse(total_partidos_condicion > 0, veces_gano_quien_anoto_primero / total_partidos_condicion, 0)
  ) %>%
...
probabilidades_ganar_agregado <- datos_goles %>%
  ...
  summarise(
    ...
    veces_gano_quien_anoto_primero_agregado = sum(PRIM_GOL == GANO, na.rm = TRUE), 
    .groups = 'drop'
  ) %>%
  mutate(
    prob_ganar_agg = ifelse(total_partidos_primer_gol_en_periodo > 0, 
                            veces_gano_quien_anoto_primero_agregado / total_partidos_primer_gol_en_periodo, 0)
  ) %>%
  arrange(P_GOL_1)
...
print(" 1 si el equipo que anotó el primer gol, ganó el partido. # 0 si el equipo que anotó el primer gol, NO ganó el partido.")
...
datos_goles_para_modelo <- datos_goles %>%
  filter(P_GOL_1 %in% c("1", "2", "3", "4", "5", "6")) %>% # Solo tiempo reglamentario
  mutate(P_GOL_1_factor = factor(P_GOL_1)) 
# Por defecto, el primer nivel ("1") será la referencia.
...
# Calcular IC para log-odds y luego exponenciar
conf_int_log_odds <- confint.default(modelo_logistico) # Usar confint.default para evitar problemas con perfiles si N es pequeño
...
Construcción de la Tabla 4 (parte inferior).
...
efecto_marcar_primero_stats <- datos_goles %>%
  ...
  group_by(TORNEO) %>%
  summarise(
    N_marco_primero_y_gano = sum(GanoQuienMarcoPrimero == 1, na.rm = TRUE), # Usar la variable ya creada
    N_con_1er_gol = n(), 
    .groups = 'drop'
  ) %>%
  mutate(
...
if(nrow(mundial_stats_efecto) == 1 && nrow(euro_stats_efecto) == 1 &&
  ...
  ), nrow = 2, byrow = TRUE) # filas=torneos, cols=gano/no gano
  
  # Usar fisher.test si las N son pequeñas, prop.test si son más grandes
  # Dado N<100 en total, Fisher es apropiado.
  test_efecto_resultado <- fisher.test(contingency_table_efecto)
  p_valor_efecto <- test_efecto_resultado$p.value
  estrellas_sig <- case_when(p_valor_efecto < 0.001 ~ "***", p_valor_efecto < 0.01 ~ "**", p_valor_efecto < 0.05 ~ "*", TRUE ~ "")
  test_efecto_details <- paste0("Fisher's Exact, p=", format.pval(p_valor_efecto, digits = 2, eps = 0.001), estrellas_sig)
}
...
for (i in 1:length(variables_cuantitativas)) {
  ...
  desc_mundial <- datos_goles %>%
    ...
    summarise(
      ...
      N = sum(!is.na(.data[[var_name]])) # N de observaciones no-NA
    )
  ...
  datos_test_mw <- datos_goles %>% 
    filter(!is.na(.data[[var_name]])) %>% # Filtrar NAs en la variable de interés
    mutate(TORNEO = factor(TORNEO)) # Asegurar que TORNEO es factor
  
  # Verificar que hay al menos dos niveles en TORNEO con datos para la variable
  if (n_distinct(datos_test_mw$TORNEO[datos_test_mw$TORNEO == "MUNDIAL"]) > 0 && 
      n_distinct(datos_test_mw$TORNEO[datos_test_mw$TORNEO == "EURO COPA"]) > 0 &&
      sum(!is.na(datos_test_mw[[var_name]][datos_test_mw$TORNEO == "MUNDIAL"])) > 0 && # Al menos una obs no-NA
      sum(!is.na(datos_test_mw[[var_name]][datos_test_mw$TORNEO == "EURO COPA"])) > 0 ) {

    # Chequear si hay varianza en los grupos (wilcox.test da error si un grupo no tiene varianza y el otro sí)
    var_mundial <- var(datos_test_mw[[var_name]][datos_test_mw$TORNEO == "MUNDIAL"], na.rm = TRUE)
    var_euro <- var(datos_test_mw[[var_name]][datos_test_mw$TORNEO == "EURO COPA"], na.rm = TRUE)

    if ( (is.na(var_mundial) || var_mundial == 0) && (is.na(var_euro) || var_euro == 0) ) {
      p_valor_mw <- 1.0 # Si ambas varianzas son 0 (o NA por N<2), no hay diferencia detectable o test no aplicable.
...
# Esta parte ya la tenías, la dejo por completitud si la quieres generar así.
...
Construcción de la Tabla 4 (parte superior).
...
datos_goles_table4_upper <- datos_goles %>%
  mutate(
    ...
    TournamentEdition = factor(TournamentEdition_Label, levels = unique(TournamentEdition_Label)[order(unique(paste0(datos_goles$TORNEO, datos_goles$YEAR)))]) # Ordena por aparición original
  )
...
if(nrow(data_for_fisher) >= 2) { 
  ...
  # Asegurar que TournamentEdition sea factor en data_for_fisher para xtabs
  data_for_fisher$TournamentEdition <- factor(data_for_fisher$TournamentEdition, levels=levels(datos_goles_table4_upper$TournamentEdition))
...
for (i in 1:length(variables_cuantitativas_labels)) {
  ...
  if (length(unique(datos_test_kw$TournamentEdition[!is.na(datos_test_kw[[var_col_name]])])) >= 2 && nrow(datos_test_kw) >=3 ) {
    if (length(unique(datos_test_kw[[var_col_name]])) > 1) { # Kruskal-Wallis necesita variación en la variable dependiente
      ...
      kw_df <- test_kw_resultado$parameters # Grados de libertad
...
# Definir el orden de las ediciones basado en el factor creado anteriormente
...
for(edicion in ediciones_ordenadas){
  if(!edicion %in% names(df_tabla4_upper_final)){
    df_tabla4_upper_final[[edicion]] <- "- (N=0)" # Rellenar si alguna edición no tuvo datos para alguna variable
  }
}
...
# ----- 3. Crear la tabla con gt -----
# (Esta parte es opcional si solo necesitas el dataframe df_tabla4_upper_final para la tabla combinada)
...
#     Test_Resultado = md("**K-W (<span class="math-inline">X^2\(p\)</span>) / Fisher (p)**") # Ajustar según los tests
#   ) %>%
#   tab_header(
...
Construcción de la Tabla 4 final combinada.

Fragmento de código

####tabla 4 final#####
# --- PASO 0: Renombrar las columnas de Test_Resultado ANTES de unir ---
# Asegurar que df_tabla4_upper_final y tabla4_parte_inferior_data están definidos
# (deben haberse creado en los chunks anteriores)
...
# Orden de las ediciones (asegurarse que datos_goles_table4_upper$TournamentEdition existe y tiene niveles)
if(!is.null(levels(datos_goles_table4_upper$TournamentEdition))) {
    ediciones_ordenadas <- levels(datos_goles_table4_upper$TournamentEdition) 
} else { # Fallback si no hay niveles (ej. si no se corrió el chunk anterior)
    ediciones_ordenadas <- unique(datos_goles_table4_upper$TournamentEdition_Label)[order(unique(datos_goles_table4_upper$YEAR))] # O una lógica similar
}
...
columnas_finales_ordenadas <- c(
  "Variable",
  ediciones_ordenadas,   # WC2018, UE2020, WC2022, UE2024 (o el orden que tengan)
  "Test_Editions",
...
se creo la tabla 4 en dos partes la A y la B. la A consiste en un kruskall para cada variable donde compara las diferentes fechas de ambos torneos juntos. mostrando una diferencia significantiva en el total de los goles marcados en el periodo 3 la B consiste en un mann whitney para cada variable donde compara los diferentes torneos. mostrando una diferencia significantiva en el total de los goles marcados en el periodo 3
