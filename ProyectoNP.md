...
se filtra la base de datos para excluir los partidos que terminaron el tiempo
...
se comprobo homocedasticidad, y a pesar de que todos los pares de variables al
comparar precentaron igualdad de varianzas, debido a la no normalidad no se
cambio de opinion en cuanto a el tipo de test que se implementara
**E.**
``` r
...
se calcularon las medidas de tendencia central(mediana debido a la no normalidad
de los datos) para cada variable y se guardaron los datos para armar la tabla 3
**G.**
...
en la tabla podemos ver un resumen de las medianas de cada año en cada variable y
los p valores correspondientes a cada variable de cada torneo donde encontramos
una diferencia significativa en los goles del periodo 3 y en el minuto del primer gol del
equipo perdedor en las fechas del mundia, y ninguna diferencia dentro de las fechas
de la eurocopa
##10. 
**A.**
...
podemos ver la probabilidad de ganar al anotar el primer gol en cada uno de los
periodos para los equipos locales y visitantes pero estos resultados no son del todo
fiables por los N demasiado pequeños en algunos subgrupos
**B.**
``` r
...
lo tanto se hizo el mismo grafico de probabilidades de ganar al anotar el primer gol en
cada periodo pero sin segregar por equipo para mejorar los N, pero de igual manera
no se consigue un resultado del todo fiable en concreto en los ultimos periodos
##11.
``` r
...
se uso la regresión logística para analizar la significancia estadística de si el período
en que se anota el primer gol afecta la probabilidad de ganar. 
(Intercept): p = 0.117.No es estadísticamente significativo a un nivel alfa de 0.05. Esto significa que el logodds de ganar cuando se anota primero en el Período 1 no es significativamente diferente de cero (lo que correspondería a odds de 1, o una probabilidad de 0.5).
P_GOL_1_factor2 a P_GOL_1_factor5: Todos tienen valores p muy altos (0.919, 0.601, 0.319, 0.952). Esto indica que no hay una diferencia estadísticamente significativa en el log odds de ganar entre anotar el primer gol en los Períodos 2, 3, 4 o 5, en comparación con anotar en el Período 1. P_GOL_1_factor6: p = 0.993. Aunque el coeficiente es enorme, el error estándar (Std. Error = 1696.73441) también es gigantesco. Esto, junto con el valor p alto, indica un problema. Como vimos en tus datos descriptivos, solo hubo 2 partidos donde el primer gol fue en el Período 6, y en ambos casos el equipo que anotó primero ganó (probabilidad del 100%). Esto se llama separación perfecta (o cuasi-perfecta) en los datos para este nivel. El modelo no puede estimar de manera confiable el efecto porque no hay variabilidad en el resultado para este grupo. El coeficiente y su error estándar son inestables y no deben interpretarse directamente.
##12.TABLA 4
``` r
...
se creo la tabla 4 en dos partes la A y la B. la A consiste en un kruskall para cada
variable donde compara las diferentes fechas de ambos torneos juntos. mostrando
una diferencia significantiva en el total de los goles marcados en el periodo 3 la B
consiste en un mann whitney para cada variable donde compara los diferentes
torneos. mostrando una diferencia significantiva en el total de los goles marcados en
el periodo 3
